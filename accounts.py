...
class Transaction(BaseModel):
    ...
    def __repr__(self):
        return (
            f"{abs(self.quantity)} acciones de {self.symbol} a {self.price} cada una."
        )
...
class Account(BaseModel):
    ...
    def sell_shares(self, symbol: str, quantity: int, rationale: str) -> str:
        ...
        if self.holdings.get(symbol, 0) < quantity:
            raise ValueError(
                f"No pueden venderse {quantity} acciones de {symbol}. No hay suficientes acciones en posesión."
            )
    ...
    def get_profit_loss(self):
        ...
        return self.calculate_profit_loss()  # type: ignore
...
if __name__ == "__main__":
    account = Account("John Doe")  # type: ignore
    account.deposit(1000)
    account.buy_shares("AAPL", 5)  # type: ignore
    account.sell_shares("AAPL", 2)  # type: ignore
